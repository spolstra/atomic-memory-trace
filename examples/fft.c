...
int main(int argc, char *argv[]) {
    ...
    while ((c = getopt(argc, argv, "p:m:n:l:stoh")) != -1) {
        switch (c) {
            ...
            case 'l':
                ...
                if (log2_line_size < 0) {
                    printerr(
                        "Log base 2 of cache line length in bytes must be >= "
                        "0\n");
                    exit(-1);
                }
                break;
            ...
            case 'h':
                ...
                printf(
                    ...
                    "transformed.\n");
                printf(
                    "  -pP : P = number of processors; Must be a power of "
                    "2.\n");
                ...
                printf(
                    "  -s  : Print individual processor timing statistics.\n");
                printf(
                    "  -t  : Perform FFT and inverse FFT.  Test output by "
                    "comparing the\n");
                printf(
                    "        integral of the original data to the integral of "
                    "the data that\n");
                printf(
                    "        results from performing the FFT and inverse "
                    "FFT.\n");
    ...
    if (rowsperproc * rootN * 2 * sizeof(double) >= PAGE_SIZE) {
        ...
        if (pages * PAGE_SIZE !=
            2 * pad_length * sizeof(double) * rowsperproc) {
            pages++;
        }
    ...
    if (dostats) {
        ...
        for (i = 1; i < P; i++) {
            ...
            if (((double)Global->transtimes[i]) / Global->totaltimes[i] >
                maxfrac) {
                maxfrac =
...

    {
        ...
        if (++(Global->start).counter != (P)) {
            ...
            while (Cycle == (Global->start).cycle) {
                Error = pthread_cond_wait(&(Global->start).cv,
                                          &(Global->start).mutex);
                if (Error != 0) {
                    break;
                }
